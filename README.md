...
- `OLLAMA_URL` (опционально) - URL Ollama сервера в формате `http://IP_АДРЕС:ПОРТ` или `http://ДОМЕН:ПОРТ`. Пример: `http://192.168.1.100:11434`
- `OLLAMA_MODEL` (опционально) - название модели Ollama, которую нужно использовать. Пример: `gemma3:1b`, `llama2`, `mistral`
...
Все ошибки логируются и отправляются пользователю с детальным описанием.

## Ограничения

- Telegram имеет лимит на длину сообщения (~4096 символов). Длинные ответы автоматически разбиваются на несколько сообщений.
